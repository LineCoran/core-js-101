...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${start}${startNum}, ${endNum}${end}`;
}
...
function isCreditCardNumber(ccn) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    const num = arr[i] * 2;
    if (i % 2 !== 0) {
      numbers.push((num > 9) ? String(num).split('').reduce((sum, acc) => sum + Number(acc), 0) : num);
    } else {
      numbers.push(arr[i]);
    }
  }
  return numbers.reduce((s, acc) => s + Number(acc), 0) % 10 === 0;
}
